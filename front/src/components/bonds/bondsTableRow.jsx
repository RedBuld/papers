...
function BondsTableRow(props)
{
    const {
        ...
        folder, isPublic, setAddToFolderModalOpen, removeFromFolder,
        row, row_index, max_index, delta, delta_dur
    } = props
    
    const bond = row
    
    const availableFolders = personalFolders.value.reduce( (result,folder) => { folder.bonds_ids.indexOf(bond.id) === -1 && result.push(folder); return result }, [])
    ...
    {
        console.log('openAddToFolderModal')

...
function Header()
{
    // const { chatUnread,lastFeatureUnread,lastHistoryUnread,logout } = useComposite()

    const [menuOpen, setMenuOpen] = useState(false)

    // const unreadFavicon = () => {
    //     if( chatUnread || lastFeatureUnread || lastHistoryUnread )
    //     {
    ...
    // }, [chatUnread,lastFeatureUnread,lastHistoryUnread])

    // {unreadMarker(lastHistoryUnread,'!')}
    // {unreadMarker(lastFeatureUnread,'!')}
    // {unreadMarker(chatUnread > 0,chatUnread)}
        ...
        <div className={"fixed top-0 left-0 z-40 w-64 h-screen p-4 overflow-y-auto transition-transform bg-white " + (menuOpen ? 'transform-none' : '-translate-x-full') } tabIndex="-1">
            ...
            <div className="pt-6 pb-2 overflow-y-auto">
                <div className="space-y-2 font-medium">
                    ...
                    <NavLink to="/history" className={({ isActive }) => "flex items-center px-2 py-2 text-sm text-center font-medium text-gray-900 border-b-2 hover:border-gray-700 " + (isActive ? "border-indigo-500" : "border-transparent") }>История рейтингов</NavLink>
                ...
                <div className="flex justify-between h-16 relative">
                    ...
                    <div className="hidden lg:flex flex-row flex-1 justify-start items-stretch">
                        <div className="flex flex-row space-x-3">
                            ...
                            <NavLink to="/history" className={({ isActive }) => "inline-flex items-center px-1 pt-1 text-sm font-medium text-gray-900 border-b-2 hover:border-gray-700 " + (isActive ? "border-indigo-500" : "border-transparent") }>История рейтингов</NavLink>
                    ...
                    <div className="flex flex-row justify-start items-stretch ml-auto">
                        <div className="flex flex-row space-x-3">
                            <NavLink to="/features" className={ ({ isActive }) => "inline-flex items-center px-1 pt-1 text-sm font-medium text-gray-900 border-b-2 hover:border-gray-700 " + (isActive ? "border-indigo-500" : "border-transparent") }>Возможности</NavLink>
                            { isLoggedIn.value && (
                            <NavLink to="/feedback" className={ ({ isActive }) => "relative inline-flex items-center px-1 pt-1 text-sm font-medium text-gray-900 border-b-2 hover:border-gray-700 " + (isActive ? "border-indigo-500" : "border-transparent") }>Обратная связь</NavLink>
                            )}
                        </div>
                    </div>
